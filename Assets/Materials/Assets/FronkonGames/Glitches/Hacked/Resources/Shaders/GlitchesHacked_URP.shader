...
Shader "Hidden/Fronkon Games/Glitches/Hacked URP"
{
  ...
  SubShader
  {
    ...
    Pass
    {
      ...
      {
        return frac(sin(dot(seed * floor(_Time.y * 30.0), float2(17.13, 3.71))) * 43758.5453123);
      }
      ...
      half4 GlitchesFrag(GlitchesVaryings input) : SV_Target
      {
        ...
        // Frame jump.
        float jump = lerp(uv.y, frac(uv.y + (_Time.y * _FrameJumpSpeed)), _FrameJump * _Strength);
        ...
        {
          float tileJitter = pixelSizeX * cos(_Time.y * _JitterSpeed) * _Jitter * _Strength;
          uvR.x += tileJitter;
          uvG.x += tileJitter;
          uvB.x += tileJitter;
        }
        ...
        // Wave jitter.
        float noise_wave_1 = snoise(float2(uv.y, _Time.y * _WaveSpeed * 20.0)) * (_Strength * _Waves * 32.0);
        float noise_wave_2 = snoise(float2(uv.y, _Time.y * _WaveSpeed * 10.0)) * (_Strength * _Waves * 4.0);
