...
namespace BuildingMakerToolset.PropPlacer
{
    ...
    public class RowPropPlacer : BuildingUnit
    {
        ...
        public List<RowPropPlacer> GetNeighborsWithScript<T>() where T : MonoBehaviour
        {
            ...
            List<T> allObjects = Object.FindObjectsByType<T>(FindObjectsSortMode.None).ToList();

            List<RowPropPlacer> rppList = Object.FindObjectsByType<T>(FindObjectsSortMode.None)
    .Where(x => x.GetComponent<RowPropPlacer>() != null)
    .Select(x => x.GetComponent<RowPropPlacer>())
    .ToList();
